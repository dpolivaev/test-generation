...
file LoginTestSuite.tsgen:
	...
	run strategy loginTests with strategy structure
...
Coverage report can be generated by adding keyword `goal` and subsection `report` to the run section.
	...
	run strategy goal loginTests with strategy structure
		apply "/java.xslt" output "generated-tests/java"
...
Strategies can include rules defined in other strategies defined in the same file, other files or libraries.
	...
	strategy loginTestsBase
		...
		let act be "submit(protocol)", "enter password(password)"

	strategy submitTest
		for each act if :act == "submit(protocol)" {
			let script be "login/LoginSubmit"
			let arrange#2 be "enter mail address(email)"
			let arrange#3 be "enter password(password)"
			let assert be "checkPage(pageAfterSubmit)"
	...
	strategy formatTest
		for each act if :act == "enter password(password)" {
			let assert be "entered password is not visible"
			let [R2] be :assert
		}

	strategy loginTests extends loginTestsBase with submitTest with formatTest

When strategy extends other strategies defined in the same file it should be placed under the referenced strategies.
...
and registering the oracle in the run section
	...
	run strategy goal loginTests with strategy structure, oracle loginOracle
		apply "/java.xslt" output "generated-tests/java"
		report  "report/testcoverage.xml"
...
and the run configuration changes to
	...
	run strategy goal loginTests with strategy structure, oracle goal loginOracle
		apply "/java.xslt" output "generated-tests/java"
		report  "report/testcoverage.xml"
